...
def filter_words(words, skip_words):
    ...
    for banned_word in skip_words:
        if words.count(banned_word) > 0:
            words.remove(banned_word)
...
def normalise_input(user_input):
    ...
    clean_string = remove_punct(user_input).lower().strip()

    #Create local list for termporary storage.
    input_list = []
    ...
    #    letterNum = 0
    #    while letterNum < len(clean_string):
    #        if clean_string[letterNum] == " " or letterNum == len(clean_string)-1:
    #            if letterNum == len(clean_string)-1:
    #                input_list.append(clean_string)
    #                break
    #            else:
    #                input_list.append(clean_string[:letterNum].strip())
    #                clean_string = clean_string[letterNum+1:].strip()
    #                if len(clean_string) > 0:
    #                    letterNum = 0
    #                else:
    #                    break
    ...
    # Filter out unnessecary words.
    return_list = filter_words(input_list, skip_words)
